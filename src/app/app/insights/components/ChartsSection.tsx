    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white/90 backdrop-blur-md p-4 rounded-xl shadow-lg border border-slate-100">
          ...
          <p className="text-sm font-semibold" style={{ color: payload[0].stroke || payload[0].fill }}>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 lg:col-span-2 relative overflow-hidden group">
         ...
         <div className="flex items-center justify-between mb-8 relative z-10">
            <div className="flex items-center gap-3">
               ...
               <div>
                  <h4 className="font-bold text-slate-800 text-lg">Product Views Trend</h4>
                  <p className="text-xs font-semibold text-slate-400">Monthly Engagement History</p>
               </div>
            </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 lg:col-span-1 border-t-4 border-t-emerald-400 flex flex-col">
         <div className="flex items-center gap-3 mb-4">
             ...
             <div>
                ...
                <p className="text-xs font-semibold text-slate-400">Distribution Overview</p>
             </div>
         </div>
            ...
            {productData.every(d => d.value === 0) ? (
                 <div className="text-center">
                    <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-3">
                       <Icon icon="solar:box-minimalistic-line-duotone" className="text-slate-300 text-3xl" />
                    </div>
                    <p className="font-bold text-slate-400">No products found</p>
                    <p className="text-xs text-slate-400 mt-1">Start by adding inventory</p>
                 </div>
            ) : (
       ...
       {topProducts && topProducts.length > 0 && (
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 lg:col-span-3 border-t-4 border-t-purple-400 relative overflow-hidden group">
             <div className="flex items-center gap-3 mb-6 relative z-10">
                 ...
                 <div>
                    <h4 className="font-bold text-slate-800 text-lg">Most Viewed (Hot) Products</h4>
                    <p className="text-xs font-semibold text-slate-400">Top performers by total lifetime views</p>
                 </div>
             </div>
             
             <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                {topProducts.slice(0, 10).map((prod, index) => {
                   const maxViews = Math.max(...topProducts.map((p) => p._count?.model_views || 1));
                      ...
                      <div key={prod.id} className="flex items-center gap-4">
                         ...
                         <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-end mb-1">
                               <p className="text-sm font-bold text-slate-700 truncate pr-2">{prod.name}</p>
                               <span className="text-xs font-black text-purple-600">{currViews}</span>
                            </div>
