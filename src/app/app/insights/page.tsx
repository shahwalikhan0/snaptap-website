  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (Admin?.id) {
           try {
             const billingRes = await api.get(`/billing/brand/${Admin.id}/current`);
             if (billingRes.data) {
        ...
        try {
           ...
           if (trendRes.data && trendRes.data.length > 0) {
             ...
             formattedTrend.sort((a: any, b: any) => a.sortKey.localeCompare(b.sortKey));
      ...
      <div className="max-w-7xl mx-auto">
          ...
          >
             <div>
                ...
                <p className="text-slate-500 font-medium ml-1">Comprehensive overview of your brand's performance metrics.</p>
             </div>
             
             <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-100">
                <Icon icon="solar:shield-check-bold-duotone" width={20} className="text-emerald-500" />
                <span className="text-sm font-bold text-slate-600">Data up to date</span>
             </div>
          </motion.div>
          ...
          <ChartsSection modelData={modelData} productData={productData} colors={COLORS} topProducts={topProducts} />
      </div>
    </div>
  );
}
